...
Conclusion why Collapse:
The fine-tuned DistilBERT model collapsed to significantly lower performance because it failed to learn the fine-grained discourse boundaries required for this task. Instead of distinguishing between intent-based labels (e.g., humor vs opinion vs analysis), the model primarily learned broad lexical and topical similarities, which caused it to merge multiple classes into a few dominant ones such as Hot Take and Informational. This issue was exacerbated by class imbalance, particularly the underrepresentation of Analytical and Meme examples, which prevented the model from forming stable decision boundaries for these categories. In addition, several labels share overlapping language (e.g., Analytical vs Hot Take and Meme vs Hot Take), making the classification signal inherently ambiguous for a small encoder model. As a result, the model overfit to surface-level keywords and defaulted to majority-pattern predictions, leading to poor generalization and a significant drop in accuracy compared to the zero-shot baseline.
